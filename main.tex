...
\textbf{(1) GNN is the dominant module for multi-class tasks.} On TBX11K, adding only the GNN head (best variant: GRAPE without VLM) yields +14.4\% F1 over the CSR baseline (0.8963 vs.\ 0.7519). The concept co-occurrence structure between the three TB finding types provides strong predictive signal that the linear head cannot exploit.
...
\begin{table}[t]
  \caption{Summary comparison: GRAPE (best variant per dataset) vs.\ CSR baseline~\cite{csr2025} (our reimpl., $^\dagger$). See Tables~\ref{tab:tbx11k} and~\ref{tab:nih} for full ablations.}
  \label{tab:compare}
  \centering
  \begin{tabular}{lcccc}
    \toprule
    ...
    \midrule
    \multirow{2}{*}{TBX11K} & CSR$^\dagger$ (Huy et al., 2025) & 0.7519 & 0.0628 & — \\
                             & GRAPE (best)                     & \textbf{0.8963} & \textbf{0.1401} & +14.4\% \\
    \midrule
